...
| **W3** | **Phase 8** ✅ | **The live battle.** `vgc.battle.entry` (a journal of taps, replayed), `/api/battles/*`, and the Battle page: fast entry, pop-ups whose first option is the likely one and which pin an ability as they log its effect, a Speed read that says *undecided* when it is, both halves of the belief side by side, WP as an average over drawn opponents with the band that says what their hidden sets are worth, undo, and a turn-by-turn walk-back. **Left:** damage snapped to calc-predicted buckets |
