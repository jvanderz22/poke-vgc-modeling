...
export function TeamLibrary({ reg, health, teams, onChanged, onUse }: {
  reg: string;
  health: Health | null;
  teams: SavedTeam[];
  ...
  async function remove(t: SavedTeam) {
    ...
    try {
      ...
      if (draft.id === t.id) { setDraft(BLANK); setCheck(null); }
      await onChanged();
    } catch (e) { setError(String((e as Error).message)); }
  }

  function edit(t: SavedTeam) {
    setDraft({ id: t.id, name: t.name, text: t.text, notes: t.notes });
    validate(t.text);
  }
      ...
      <div className="panel">
        ...
        {teams.map((t) => (
          <div key={t.id} className="team-row" aria-current={draft.id === t.id}>
            ...
            <button className="link name" onClick={() => edit(t)}>{t.name}</button>
            <button className="link tiny" onClick={() => onUse(t)}>use</button>
            <button className="link tiny err" onClick={() => remove(t)}>×</button>
          </div>
        ))}
        <button className="ghost" style={{ marginTop: 8 }} onClick={() => { setDraft(BLANK); setCheck(null); }}>
          New team
        </button>
      </div>
