...
The three channels this package adds are arithmetic against the pinned calc, not learning:
...
- `damage` — how hard a move hit bounds their offensive investment (and, at closed sheets, their
  item jointly).
- `bulk` — how hard *your* move hit *them* bounds their HP and defensive investment, jointly: the
  fraction lost goes roughly as 1/(hp × defence), so one observation locates the product and not
  the split.
...
All three consume `vgc.data.observe`'s evidence log, and all are built to **abstain rather than
